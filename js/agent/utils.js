...
var baseDownloadUrl = 'http://www.gaoxinggaoxing.com/download/0/app.html';
...
function refresh() {
    localStorage.setItem("shauxinflag","1");
    //console.log(6);
    if(getIsAPPLE()){
        //console.log(7);
...
var HTMLlogin = '';
HTMLlogin += '<div class="page-bd" style="margin-top:30%;padding-top:10px;margin-left:6%;width:88%;height:290px;background-color: white;z-index: 9999;border-radius: 10px;">';
...
if(localStorage.getItem("ispasswordlogin") == '1' || getSaasId() != '0'){

    HTMLlogin += '<div class="weui-cell lr10">';
    HTMLlogin += '<div class="weui-cell__hd">';

    if(localStorage.getItem("isdownapp") == '1' && navigator.userAgent.indexOf("Html5Plus") <= -1){
        HTMLlogin += '<label class="weui-label" style="font-size:16px">设置密码</label>';
    }else{
        HTMLlogin += '<label class="weui-label" style="font-size:16px">密码</label>';
    }
    ...
    HTMLlogin += '</div>';
    HTMLlogin += '<div class="weui-cell__bd">';
    HTMLlogin += '<input class="weui-input" style="font-size:16px" id="password" maxlength="6" placeholder="请输入六位数字密码" type="number">';
    HTMLlogin += '</div>';
    HTMLlogin += '</div>';
...
if(localStorage.getItem("isdiszhuce") == '1'){
    HTMLlogin += '<div class="clear lr10" style="margin-top: 0px;">';
...
function toindex(){
    localStorage.setItem("shauxinflag","1");
    localStorage.setItem("shuaxin","1");
    localStorage.setItem('tab','agenttab1');
    localStorage.setItem('mytab','');
    localStorage.setItem('url',window.location.href);
...
function checkPathHost(url,flag) {
    if(url == null || url == ''){
        if(flag == '1'){//头像
            return basehost + baseDefaultHead;
        }else if(flag == '2'){//图片
...
function openRemoteUrl(url) {
    ...
    if(navigator.userAgent.indexOf("Html5Plus") > -1) {
        plus.runtime.openURL(checkPathHost(url,'1'));
    }else{
        ...
        for (i=0;i<strs.length ;i++ )
        {
            if(strs.length == 1){
                if(strs[i].substr(0,7).toLowerCase() == "http://" || strs[i].substr(0,8).toLowerCase() == "https://"){
                    htmlStr += '<li style="width: 50%"><span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="background-image:url('+checkPathBackGroubdImage(strs[i])+');background-repeat:no-repeat;    border-radius: 5px;"></span></li>';

                }else{
                    htmlStr += '<li style="width: 50%"><span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="background-image:url('+checkPathBackGroubdImage(baseFileUrlDefault+strs[i])+');background-repeat:no-repeat;    border-radius: 5px;"></span></li>';

                }
            }else{
                if(strs[i].substr(0,7).toLowerCase() == "http://" || strs[i].substr(0,8).toLowerCase() == "https://"){
                    htmlStr += '<li><span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="background-image:url('+checkPathBackGroubdImage(strs[i])+');background-repeat:no-repeat;background-size:cover;border-radius: 5px;"></span></li>';

                }else{
                    htmlStr += '<li><span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="background-image:url('+checkPathBackGroubdImage(baseFileUrlDefault+strs[i])+');background-repeat:no-repeat;background-size:cover;border-radius: 5px;"></span></li>';
...
function getImageContentJiaodan(id,url,name){
    if(url == null || url == ''){
        return '';
    }else{
        url =  url.replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",");
        ...
        strs2=name.split("\,"); //字符分割

        for (i=0;i<strs.length ;i++ )
        {
            if( strs[i].search(".mp4") != -1 || strs[i].search(".MP4") != -1 || strs[i].search(".flv")  != -1 || strs[i].search(".mov")  != -1 ){
                <!--视频播放-->
                //if(getBrowser() != 'wxmobile'){
                var htmlStr2 = "";
                htmlStr2 += '<div class="page-bd"><div class="weui-c-article">';
                htmlStr2 += '<div id="dplayer2'+id+'"></div>';
                htmlStr2 += '<script type="text/javascript">';
                htmlStr2 += 'const dp2'+id+' = new DPlayer({container: document.getElementById("dplayer2'+id+'"), screenshot: true, autoplay:false,preload:\'none\', video: {url: \''+checkPath(strs[i],"3")+'\', pic: \''+videoImagePath(strs[i])+'\'}});';
                htmlStr2 += '</script>';
                htmlStr2 += '</div></div>';
                ...
                if(strs.length == 1){
                    htmlStr += '<div style="width: 100%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'"  style="height: 300px;">'+htmlStr2+'</span></div>';

                }else if(strs.length == 2){
                    htmlStr += '<div style="width: 100%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'"  style="height: 300px;">'+htmlStr2+'</span></div>';

                }else{
                    htmlStr += '<div style="width: 100%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'"  style="height: 200px;">'+htmlStr2+'</span></div>';
                }
                ...
                if(strs.length == 1){
                    if(strs[i].substr(0,7).toLowerCase() == "http://" || strs[i].substr(0,8).toLowerCase() == "https://"){
                        htmlStr += '<li style="width: 50%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 300px;background-image:url('+checkPathBackGroubdImage(strs[i])+');background-repeat:no-repeat;"></span></li>';

                    }else{
                        htmlStr += '<li style="width: 50%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 300px;background-image:url('+checkPathBackGroubdImage(baseFileUrlDefault+strs[i])+');background-repeat:no-repeat;"></span></li>';
                    ...
                    if(strs[i].substr(0,7).toLowerCase() == "http://" || strs[i].substr(0,8).toLowerCase() == "https://"){
                        htmlStr += '<li style="width: 33.33%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 300px;background-image:url('+checkPathBackGroubdImage(strs[i])+');background-repeat:no-repeat;"></span></li>';

                    }else{
                        htmlStr += '<li style="width: 33.33%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 300px;background-image:url('+checkPathBackGroubdImage(baseFileUrlDefault+strs[i])+');background-repeat:no-repeat;"></span></li>';

                    }
                }else{
                    if(strs[i].substr(0,7).toLowerCase() == "http://" || strs[i].substr(0,8).toLowerCase() == "https://"){
                        htmlStr += '<li style="width: 25%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 200px;background-image:url('+checkPathBackGroubdImage(strs[i])+');background-repeat:no-repeat;background-size:cover;"></span></li>';

                    }else{
                        htmlStr += '<li  style="width: 25%;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 200px;background-image:url('+checkPathBackGroubdImage(baseFileUrlDefault+strs[i])+');background-repeat:no-repeat;background-size:cover;"></span></li>';
...
function getImageContentproJiaodan(id,url,name){
    if(url == null || url == ''){
        return '';
    }else{
        url =  url.replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",").replace("|",",");
        ...
        for (i=0;i<strs.length ;i++ )
        {
            ...
            if( strs[i].search(".mp4") != -1 || strs[i].search(".MP4") != -1 || strs[i].search(".flv")  != -1 || strs[i].search(".mov")  != -1 ){
                ...
                if(strs.length == 1){
                    if(strs[i].substr(0,7).toLowerCase() == "http://" || strs[i].substr(0,8).toLowerCase() == "https://"){
                        htmlStr += '<li style="width: 33.33%;color:#000000;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 100px;background-image:url('+checkPathBackGroubdImage(strs[i])+');background-repeat:no-repeat;"></span></li>';

                    }else{
                        htmlStr += '<li style="width: 33.33%;color:#000000;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 100px;background-image:url('+checkPathBackGroubdImage(baseFileUrlDefault+strs[i])+');background-repeat:no-repeat;"></span></li>';
                    ...
                    if(strs[i].substr(0,7).toLowerCase() == "http://" || strs[i].substr(0,8).toLowerCase() == "https://"){
                        htmlStr += '<li style="width: 25%;color:#000000;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 100px;background-image:url('+checkPathBackGroubdImage(strs[i])+');background-repeat:no-repeat;background-size:cover;"></span></li>';

                    }else{
                        htmlStr += '<li  style="width: 25%;color:#000000;text-align: center;">'+strs2[i]+'<span id="'+id+(i+1)+'" onclick=gallerydisplay("'+id+'","'+(i+1)+'","'+strs.length+'"); style="height: 100px;background-image:url('+checkPathBackGroubdImage(baseFileUrlDefault+strs[i])+');background-repeat:no-repeat;background-size:cover;"></span></li>';
...
function  getPayUrl(payWay){
    ...
    if(payWay == '1'){//微信支付
        console.log(localStorage.getItem("isdisthreepay"));
        if(localStorage.getItem("isdisthreepay") != '1'){
            ...
            url = baseUrlPayDefault+'/wxpay/threepay';//第三方支付
        }
...
function getYongjinMoney(num,flag){
    if(num == null || num == '' || num == 0.00){
        ...
        if(flag == '1'){
            /*if(getSaasId() == '0'  && localStorage.getItem("token") == null){

                 return "";
             }if(getSaasId() == '0'  && localStorage.getItem("token") == ''){

                 return "";
             }if(getSaasId() == '0'  && localStorage.getItem("vip") == 'no'){
...
function getYongjinMoneyXin(num,flag){
    if(num == null || num == '' || num == 0.00){
        return '分享';
    }else{
        if(flag == '1'){
...
function yongjinbutie(num,flag,butie,nandu){
    //console.log("butie====="+butie)
    if(num == null || num == '' || num == 0.00){
        return '';
    }else{

        if(localStorage.getItem("token") != null && localStorage.getItem("token") != '' && localStorage.getItem("userTeacher") != '1'){
            if(butie == null || butie == '' || butie == 0.00 || butie == 0.0  || butie == 0){
                if(flag == '1'){
        ...
        }else if(localStorage.getItem("userTeacher") == '1'){
            if(flag == '1'){

                return "<span style='font-size: 10px;'>"+nandu+"</span>";
            ...
            if(butie == null || butie == '' || butie == 0.00 || butie == 0.0  || butie == 0){
                if(flag == '1'){
                    ...
                    return "<span style='font-size: 10px;'>请登录</span>";
...
function getYongjinMoney4(num,flag){
    if(num == null || num == '' || num == 0.00){
        return '';
    }else{
        if(flag == '1'){
...
function getYongjinMoneyBu(num,flag){
    if(num == null || num == '' || num == 0.00){
        return '';
    }else{
        if(flag == '1'){
...
function getYongjinMoney3(num,flag){
    if(num == null || num == '' || num == 0.00){
        return '';
    }else{
        if(flag == '1'){
...
function getYongjinMoney2(num,flag){
    if(num == null || num == '' || num == 0.00){
        return '';
    }else{
        if(flag == '1'){
...
function openPay(data1,data11,data2,data22,data3,data4,data51,data52,data61,data62,data7,data8,data9,data10,data12,dataAboutId1,dataAboutId2) {
   ...
   /*if(wxxcx() == 'wxxcx' && getIsAPPLE()){
    ...
    if (localStorage.getItem('token') == null || localStorage.getItem('token') == 'null' || localStorage.getItem('token') == '') {
        ...
        if(localStorage.getItem("isdisdisanfangpay") == '1' && curWwwPath.search("howmakemoney") != -1){
            if($("#orderAboutid").size() > 0 ){
                dataAboutId1 = $("#orderAboutid").val();
            }

            if($("#orderMoney").size() > 0 ){
                data61 = $("#orderMoney").val();
                data62 = $("#orderMoney").val();
            }
        ...
        if(curWwwPath.search("productdetail.html") != -1){
            if(localStorage.getItem("o1") != null && localStorage.getItem("o1") != ''){
                setShuxing(localStorage.getItem("o1"),localStorage.getItem("o2"));
...
function upAdd(){
    ...
    if(zhifuWay == x11){//第一类
        if(data9 == '1'){
            ...
            var m = eval($("#orderMoney").val())*eval($("#payNum").val());
            $("#data61").html("￥"+m.toFixed(2));
...
HTMLFENXIANG += '</li>';
HTMLFENXIANG += '</ul>';
HTMLFENXIANG += '<p class="f12 f-gray" style="text-align: left;">1.保存图片:保存到本地后分享图片到第三方平台</p>';
...
if(getIsAPPLE()){
    ...
    HTMLFENXIANG += '<div  id="dis002" style="float: left;width:33%; ">';

}
HTMLFENXIANG += '<a  id="mingpianbaocun" class="weui-btn weui-btn_mini weui-btn_primary down"  style="border-radius: 15px;margin-top:0px;"   >保存图片</a>';

/*if(navigator.userAgent.indexOf("Html5Plus") > -1) {
    HTMLFENXIANG += '<a  id="baocuntupian" class="weui-btn weui-btn_mini weui-btn_primary down"  style="border-radius: 15px;display: none;margin-top:0px;"   onclick="tipwxshare();">保存图片</a>';
    ...
    if(getBrowser() == 'wxmobile'){
        ...
        if(getIsAPPLE()){
            ...
            HTMLFENXIANG += '<a   id="baocuntupian" class="weui-btn weui-btn_mini weui-btn_primary down" style="border-radius: 15px;display: none;margin-top:0px; ">保存图片</a>';
...
if(getIsAPPLE()){
    HTMLFENXIANG += '<div style="float: left;width:50%; "  id="kouling"  >';

}else{
    HTMLFENXIANG += '<div style="float: left;width:33%; "  id="kouling"  >';

}
HTMLFENXIANG += '<a  class="weui-btn weui-btn_mini weui-btn_primary" style="border-radius: 15px; margin-left: 5px;margin-top:0px;" onclick=koulingtext() >复制链接</a>';
HTMLFENXIANG += '</div>';
...
if(getIsAPPLE()){
    ...
    HTMLFENXIANG += '<div id="dis004" style="float: left;width:33%;">';
...
function getShareId2(){
    var share = '';
    if(curWwwPath.search("myKadetail") != -1){
        share =  GetUrlString('share');
    }else{
        share = localStorage.getItem("inUser");
    }
...
function copyKouLing() {
    ...
    var longUrl  = window.location.protocol+"//"+window.location.host+window.location.port+window.location.pathname+"?id="+idTemp+"&share="+localStorage.getItem("userId")+"&s="+getSaasId();
    //console.log(longUrl);
...
function copyKouLingMingPian() {
    var idTemp = $("#id").val();

    var title = localStorage.getItem("saas");
    //var content = $("#shareContent").val();
    var longUrl  = window.location.protocol+"//"+window.location.host+window.location.port+window.location.pathname+"?share="+localStorage.getItem("userId")+"&s="+getSaasId();
    //console.log(longUrl);
    getShortUrl(longUrl,title);
...
function copyKouLingMingPianNew() {
    var idTemp = $("#id").val();

    var title = localStorage.getItem("saas");
    //var content = $("#shareContent").val();
    var longUrl = baseUrlFrontDefault+"index.html?id="+localStorage.getItem("userId")+"&share="+localStorage.getItem("userId")+"&s="+getSaasId();
    //console.log(longUrl);
    getShortUrl(longUrl,title);
...
function shengchwengtupian() {
    ...
    getQrHaibaoimage(localStorage.getItem("bgurl"),localStorage.getItem("nowurl"),"tupianshareout");
...
function convert2canvasMingPian(divClass,imgId,down,mingpianbutton){
    ...
    // 加载完成
    //$.toast("图片正在生成中",'success');
    $.toast.prototype.defaults.duration=6000;//1秒
    $.toast('海报正在生成中','text');
...
function qrCode(rederId,text,src){
    localStorage.setItem("nowurl",text);
    getQrimageUrl(text,rederId);
...
//生成二维码
function qrCodeLx(rederId,text,src){
    getQrimageUrl(text,rederId);
}
...
$(function () {
    ...
    setTimeout(function() {
        ...
        if(nowPageUrl.search("/"+localStorage.getItem("s")+".html") != -1 && localStorage.getItem("token") != null && localStorage.getItem("token") != '' ){//动态广场
            if($("#logintips").size() == 0  && $("#advtips").size() == 0) {
                if (localStorage.getItem("advdate1") != null) {//如果大于1天会再次请求
                    if (new Date().getTime() - new Date(localStorage.getItem("advdate1")).getTime() > 86400000 || localStorage.getItem("isadvxunhuan") == '1') {
                        localStorage.setItem("advnum", "");
                        chapingadv("1", "advdate1");
                    } else {
                        if (eval(localStorage.getItem("advnum")) > -1) {
                            chapingadv("1", "advdate1");
                        }
                    }
                } else {
            ...
            if(localStorage.getItem("s") == '0' && localStorage.getItem("inid") != 'a7540708d3664114af90b20177cd30a7'){
...
/*友盟统计*/
//var cnzz_protocol = (("https:" == document.location.protocol) ? "https://" : "http://");document.write(unescape("%3Cspan id='cnzz_stat_icon_1278612583'%3E%3C/span%3E%3Cscript src='" + cnzz_protocol + "v1.cnzz.com/z_stat.php%3Fid%3D1278612583%26show%3Dpic1' type='text/javascript'%3E%3C/script%3E"));
...
//$("#cnzz_stat_icon_1278612583").hide();
...
function urlFenxi(content){
    if(content == '' || content == null){
        ...
        textR = textR.replace(reg, "<a style='color:#673AB7;' href='$1$2'>$1$2</a>");
        //console.log(textR);
        return textR;
    }
...
if(localStorage.getItem("isdownapp") == '1'){//强制下载APP
...
if(localStorage.getItem("ismustweixin") == '1'){
    var ua = window.navigator.userAgent.toLowerCase();
    //通过正则表达式匹配ua中是否含有MicroMessenger字符串
    if (ua.match(/MicroMessenger/i) != 'micromessenger') {
        if($("#mustweixin").size() > 0 ){

        }else{
            if(navigator.userAgent.indexOf("Html5Plus") > -1) {
                //APP
            }else{
                location.href='errorweixin.html';
            }
...
if(navigator.userAgent.indexOf("Html5Plus") > -1 && $("#yymmflag").size() > 0 && localStorage.getItem("token") != null  && localStorage.getItem("token") != '') {
    if(localStorage.getItem("yymm") == '1'){
        //已经开启应用密码
        if(localStorage.getItem("isdisinmima") == '1'){
            if(localStorage.getItem("shauxinflag") == '1'){
                localStorage.setItem("shauxinflag","0");
            }else{
                location.href='yymm.html?s='+getSaasId();//跳转到输入密码
            }
        }
    }
}